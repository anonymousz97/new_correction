...
class PhoBertEncoder(nn.Module):
	def __init__(self, n_words: int, 
		...
		self.tokenizer = AutoTokenizer.from_pretrained(BERT_PRETRAINED)
	...
	def merge_embedding(self, sequence_embedding: Tensor, sequence_split, mode='add'):
		sequence_embedding = sequence_embedding[: sum(sequence_split)]  # batch_size*seq_length*hidden_size
