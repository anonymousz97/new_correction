...
class ModelTrainer:
	def __init__(self, model_name, n_samples,
		...
		self.device = DEVICE
		self.token_ids = load_file_picke('sample_dataset.pkl')
		...
		if self.is_continuous_train:
			...
			print("LOADED DONE")
		self.pos_weight = torch.randint(1, 10, (1,)).float()
		print("POS WEIGHT ", self.pos_weight)
		self.loss_detection = nn.BCEWithLogitsLoss(weight=self.pos_weight)
	...
	def get_loss_detection(self, detection_outputs, label_errors):
		label_errors = label_errors.float().cpu()
		detection_outputs = detection_outputs.squeeze(-1).cpu()
		loss1 = self.loss_detection(detection_outputs, label_errors)
	...
	def train_one_epoch(self):
		...
		for idx, data in tqdm(enumerate(self.train_loader)):
			...
			if self.is_bert:
				loss = self.get_loss_detection(detection_outputs, label_errors)
			else:
	...
	def evaluate_model(self):
		...
		# correction
		total_prediction_correction_correct = 0
		if not self.is_bert:
